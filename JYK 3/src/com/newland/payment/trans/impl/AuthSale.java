 ...
 * @time 上午11:08:12
 */
public final class AuthSale extends AbstractBaseTrans {
	...
	public AuthSale(ThirdInvokeBean thirdInvokeBean) {
	...
	protected int init() {
		...
		pubBean.setTransType(TransType.TRANS_AUTHSALE);
		pubBean.setTransName(getTitle(pubBean.getTransType()));
		
		if (ParamsUtils.getBoolean(
				ParamsConst.PARAMS_KEY_IS_AUTH_SALE_PIN)) {
			isInputPin = true;
		} else {
			isInputPin = false;
		}
	...
	public int step_transStart(){
		if(ParamsUtils.getString(ParamsConst.PARAMS_KEY_AUTH_SALE_MODE).equals("2")){
			MessageUtils.showTipMsg(getText(R.string.error_not_authsale));
			return FINISH;
		}
