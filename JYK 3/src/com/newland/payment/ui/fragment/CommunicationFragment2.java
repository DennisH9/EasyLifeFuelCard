...
public class CommunicationFragment2 extends BaseFragment {
	...
	private CommunicationFragment2() {
		super(ParamsUtils.getInt(ParamsConst.PARAMS_KEY_COMMUNICATION_OUT_TIME, 60));
	}
	
	public static CommunicationFragment2 newInstance(CommunicationBean communicationBean) {
		CommunicationFragment2 fragment = new CommunicationFragment2();
	...
	private void startCommunication(){
		new Thread(){
			public void run(){
				...
				byte[] requst = communicationBean.getData().getBytes();
				LoggerUtils.d("hjh.....requst"+BytesUtils.bytesToHex(requst));
				byte[] response = null;
				// 发送数据
				if (ParamsUtils.getBoolean(ParamsConst.PARAMS_KEY_IS_USE_HTTP)
					...
					? socketHelper.receiveSLL():socketHelper.receive2();
				...
				isSucessFlag = true;
				communicationBean.setData(BytesUtils.bytesToHex(response));
				//处理报文头请求
				onSucess();
			}
		}.start();
	...
	private void trySocketConnect() {
		...
		String ip;
		int port;
		LoggerUtils.d("hjh.....PARAMS_IS_PAY_QUERY");
		if(ParamsUtils.getInt(ParamsConst.PARAMS_KEY_COMMUNICATION_TYPE) == 0){
			ip = ParamsUtils.getString(ParamsConst.PARAMS_KEY_CDMA_SERVERIP2);
			port = ParamsUtils.getInt(ParamsConst.PARAMS_KEY_CDMA_PORT2, 8080);			
		}else if(ParamsUtils.getInt(ParamsConst.PARAMS_KEY_COMMUNICATION_TYPE) == 1){
			ip = ParamsUtils.getString(ParamsConst.PARAMS_KEY_GPRS_SERVERIP2);
			port = ParamsUtils.getInt(ParamsConst.PARAMS_KEY_GPRS_PORT2, 8080);			
		}else{
			ip = ParamsUtils.getString(ParamsConst.PARAMS_KEY_WIFI_SERVERIP2);
			port = ParamsUtils.getInt(ParamsConst.PARAMS_KEY_WIFI_PORT2, 8080);					
		}
